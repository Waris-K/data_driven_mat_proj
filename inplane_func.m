...
function [U_out] = inplane_func(matDist,rf)

%% load mat distribution
dataStruct = load(matDist);
fieldName = fieldnames(dataStruct); % Get the variable name(s)
material_Distribution = dataStruct.(fieldName{1}); % Extract the variable

%% refine 
% Refinement Factor
refinementFactor = rf;  % Adjust to refine mesh (e.g., 2 = 16x16 elements)
...
for samples = 1:length(material_Distribution)
...
for row = 1:numElementsY
    for col = 1:numElementsX
        matType = material_Distribution(row, col, samples); % Get material type
        ...
        new_material_distribution(newRowStart:newRowStart+refinementFactor-1,newColStart:newColStart+refinementFactor-1) = matType;
    end
end
...
% %% plotting (ingnor for the function)
...
% Ux_out = zeros(length(Ux),1,length(material_Distribution));
% Uy_out = zeros(length(Ux),1,length(material_Distribution)); 
