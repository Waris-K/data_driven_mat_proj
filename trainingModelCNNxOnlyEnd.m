...
displacement = displacement(end-8:end,:); 
outputSize = 9; 
...
options = trainingOptions('adam', ...
    ...
    'Verbose', true,'Plots', 'training-progress');  % Disable verbose output for training

%% Train the network
[netXlast,info] = trainNetwork(trainData, trainLabels, layers, options);


%% saving the training

% Save the trained network
save('trainedCNNModelxOnlylast.mat', 'netXlast');

%% load the network
load('trainedCNNModelxOnlylast.mat')

%% Evaluate the Validation

% Evaluate the model on the validation data
predictedValidationLabels = predict(netXlast, valData);
